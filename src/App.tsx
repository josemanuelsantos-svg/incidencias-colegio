        ...
        <div className="absolute inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-6 text-center animate-in fade-in">
          <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-gray-300 text-sm mb-6 leading-relaxed">
              Se activará la cámara para que puedas{" "}
              <strong>enfocar la escena en vídeo</strong>.
              <br />
              <br />
              Cuando tengas el encuadre, pulsa el botón para{" "}
              <strong>congelar el momento</strong> y guardarlo como foto.
            </p>
...
const StepCategory = ({ updateForm, handleNext }) => (
  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <button
        onClick={() => {
          updateForm("category", "maintenance");
          handleNext();
        }}
      ...
      >
        ...
        <div className="text-center">
          ...
          <span className="text-sm text-orange-600">
            Luces, muebles, puertas...
          </span>
        </div>
      </button>
      <button
        onClick={() => {
          updateForm("category", "it");
          handleNext();
        }}
...
const StepLocation = ({ formData, updateForm, handleNext, handleBack }) => {
  ...
  const canContinue = isCommonSpace
    ...
    <div className="space-y-5 animate-in fade-in slide-in-from-right-8 duration-300">
              ...
              {availableFloors.map((f) => (
                <button
                  ...
                  className={`flex-1 p-2 text-sm border rounded-lg ${
                    formData.floor === f ? "bg-blue-600 text-white" : "bg-white"
                  }`}
                >
                  {f}
                </button>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          ...
          className="flex-1 bg-gray-900 text-white rounded-lg disabled:opacity-50"
        >
          Siguiente
        </button>
      </div>
  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-right-8 duration-300">
      ...
      <div className="bg-gray-50 p-3 rounded-lg border text-sm flex gap-2 text-gray-600">
        ...
        <span>
          {formData.building}{" "}
          {formData.floor !== "N/A" && `• ${formData.floor}`} •{" "}
          <b>{formData.room}</b>
        </span>
      </div>
        ...
        <div className="flex gap-2">
          {["normal", "urgente"].map((p) => (
            <button
              ...
              className={`flex-1 py-1.5 text-sm border rounded capitalize ${
                formData.priority === p
                  ? p === "urgente"
...
const Dashboard = ({ incidents }) => {
  ...
  const maintCount = incidents.filter(
  ...
  const topBuilding =
    Object.keys(buildingCounts).sort(
      (a, b) => buildingCounts[b] - buildingCounts[a]
    )[0] || "-";
    ...
    <div className="space-y-6 pb-20 animate-in fade-in">
      ...
      <div className="bg-white p-4 rounded-xl border shadow-sm space-y-4">
        <h3 className="font-bold text-gray-700 flex items-center gap-2">
          <BarChart3 size={18} /> Por Categoría
        </h3>
          ...
          <div className="flex-1">
            ...
            <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(maintCount / total) * 100}%` }}
              ></div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border shadow-sm">
        ...
        <div className="text-lg font-bold text-gray-800 mt-1 truncate">
          {topBuilding}
        </div>
      </div>
    </div>
...
const IncidentsList = ({
  ...
  const filtered = incidents.filter((i) => {
    ...
    const matchText =
      ...
      (i.reporterName &&
        i.reporterName.toLowerCase().includes(filterText.toLowerCase()));
    const matchBuilding =
      filterBuilding === "all" || i.building === filterBuilding;
    const matchMine =
      !onlyMyIncidents || (savedName && i.reporterName === savedName);
        ...
        <div className="flex gap-2 text-xs overflow-x-auto pb-1">
          <button
            ...
            className={`px-3 py-1 rounded-full border ${
              filterStatus === "all" ? "bg-gray-800 text-white" : "bg-white"
            }`}
          >
            Todas
          </button>
            ...
            >
              <div className="flex justify-between items-start mb-2">
                ...
                {isAdmin ? (
                  <div className="flex gap-2">
                    {incident.status === "pending" ? (
                      <button
                        onClick={() => initiateResolve(incident.id)}
                        className="text-xs font-bold px-3 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 shadow-sm"
                      >
                        ✓ Resolver
                      </button>
                    ) : (
                      <button
                        ...
                        className="text-xs font-bold px-3 py-1.5 bg-white border border-gray-300 text-gray-600 rounded-lg hover:bg-gray-50"
                      >
                        Reabrir
                      </button>
                    )}
                    <button
                      onClick={() => initiateDelete(incident.id)}
                  ...
                  {incident.reporterName && (
                    <>
                      ...
                      <span className="flex items-center gap-1 text-gray-500 font-medium">
                        <User size={10} /> {incident.reporterName}
                      </span>
                    </>
                  )}
                </div>
              ...
              <div className="flex flex-wrap gap-2 text-xs text-gray-500 mt-2">
                <span className="flex items-center gap-1 bg-white border px-2 py-1 rounded">
                  <Building size={10} /> {incident.building}
                </span>
                <span className="flex items-center gap-1 bg-white border px-2 py-1 rounded">
                  <span className="font-bold">{incident.room}</span>
                </span>
                {incident.imageData && (
                ...
                <div className="mt-3 bg-green-50 p-2 rounded-lg border border-green-100 text-xs text-green-800 flex gap-2 items-start">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-green-600 mt-1">
                      {getRelativeTime(incident.resolvedAt)}
                    </div>
                  </div>
                </div>
              )}
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        // Intentamos autenticación con token (si existe y coincide con la config)
        if (
          ...
          try {
            ...
            return; // Éxito con token
          } catch (tokenError) {
            // Si el token falla (ej: config propia distinta a la del entorno), ignoramos y seguimos a anónimo
            console.log(
              "Token de entorno no válido para esta configuración, probando anónimo..."
            );
          }
        }
        // Fallback: Autenticación anónima (necesaria para proyectos propios)
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      (formData.room === "Pasillo / Baños" || formData.room === "Otro") && // Ajuste por si varía
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `incidencias_escolares_${new Date().toISOString().slice(0, 10)}.csv`
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 pb-10">
        ...
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title={
                isAdmin ? "Salir de Administración" : "Acceso Administración"
              }
            >
      ...
      <main className="max-w-2xl mx-auto p-4 md:p-6">
        {activeTab === "new" && (
          <div className="max-w-lg mx-auto bg-white md:shadow-lg md:rounded-2xl md:p-8 md:border min-h-[50vh]">
            {step === 1 && (
              <StepCategory updateForm={updateForm} handleNext={handleNext} />
            )}
            {step === 2 && (
              <StepLocation
                formData={formData}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t px-6 py-3 flex justify-around z-20 shadow-lg pb-6">
        <button
          ...
          className={`flex flex-col items-center gap-1 ${
            activeTab === "new" ? "text-blue-600" : "text-gray-400"
          }`}
        >
          <PlusCircle size={24} />
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm animate-in zoom-in-95">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Añade una nota sobre la solución (opcional):
            </p>
            <textarea
              value={resolveNote}
              onChange={(e) => setResolveNote(e.target.value)}
              placeholder="Ej: Pieza cambiada, tornillo apretado..."
      ...
      <div className="text-[1px] text-gray-100 select-none text-center">
        Хэрэв та үүнийг олвол та бас монголоор ярьдаг гэсэн үг
      </div>
    </div>
  );
